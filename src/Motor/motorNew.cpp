...
static float angleSpeedDeepFiltered_epm;
static float angleSpeedLowFiltered_epm;
...
public:
    ...
    Filter(const float g, const float k) : gain(g), k(k) {}
};
...
logmin  =
 */
static Filter angleSpeedDeepFilter{6.583695028e+03F, 0.9996962192F};
...
logmin  =
 */
static Filter angleSpeedLowFilter{6.592690015e+02F, 0.9969663370F};
...
static float filterLow(const float w)
{
    return angleSpeedLowFilter.run(w);
}
static float filterDeep(const float w)
{
    return angleSpeedDeepFilter.run(w);
}
...
static void processAngleSpeedMeasure()
{
    ...
    angleSpeedDeepFiltered_epm = filterDeep(angleSpeed_epm);
    angleSpeedLowFiltered_epm = filterLow(angleSpeed_epm);
}
...
static float getStreamValueForLogChannel(const int nChannel)
{
    ...
    switch (nStream) {
        ...
        case 18:    return angleSpeedDeepFiltered_epm;
        case 19:    return pi::getIntegrator(piAngleSpeed);
        case 20:    return pi::getIntegrator(piId);
        case 21:    return pi::getIntegrator(piIq);
        case 22:    return angleSpeedLowFiltered_epm;
        default:    return 0.0F;
    }
}
...
static void doCompensationCrossCoupling()
{
    ...
    const float w = angleSpeedLowFiltered_epm * 2.0F * 3.1415F / 60.0F;
...
{
    return angleSpeedDeepFiltered_epm;
}
